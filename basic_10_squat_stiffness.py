...
# Updated rotate_point_with_quaternion with type-checking and error handling
def rotate_point_with_quaternion(qx, qy, qz, qw, point):
    point = np.array(point)  # Ensure it's a numpy array
    if point.shape != (3,):
        raise ValueError(f"[❌] Invalid point shape: expected (3,), got {point.shape}, value: {point}")
...
# Updated calculate_cartesian_coordination with safe row-wise rotation
def calculate_cartesian_coordination(df):
    start_point = np.array([1, 0, 0])  # unit vector along x-axis
    rotated_points = []
    ...
    for idx, row in df.iterrows():
        try:
            ...
            print(f"[⚠️] Rotation failed at index {idx}: {e}")
            rotated_points.append([np.nan, np.nan, np.nan])  # or you could skip or interpolate

    rotated_points = np.array(rotated_points)

    df['x_after_rotate'] = rotated_points[:, 0]
    df['y_after_rotate'] = rotated_points[:, 1]
    df['z_after_rotate'] = rotated_points[:, 2]

    # Calculate velocities and accelerations
    z_abs_velocity = np.abs(np.diff(df['z_after_rotate'])) / 0.005
    z_abs_velocity = np.append(z_abs_velocity, 0)
    df['z_abs_velocity'] = z_abs_velocity

    z_velocity = np.diff(df['z_after_rotate']) / 0.005
    z_velocity = np.append(z_velocity, 0)
    df['z_velocity'] = z_velocity

    z_acceleration = np.diff(z_velocity) / 0.005
    z_acceleration = np.append(z_acceleration, 0)
    df['z_acceleration'] = z_acceleration

    # Filter rows based on velocity threshold
    threshold = 0.4
    valid_rows = df[df["z_abs_velocity"] > threshold]
    if valid_rows.empty:
        raise ValueError("[❌] No rows passed the velocity threshold filtering.")
    
    first_valid_index = valid_rows.index[0]
...
def calculate_linear_regression(df, x_column, v_column, x_column_quadratic, x_column_cubic, x_column__quartic, a_column):
    ...
    x1 = df[[x_column, v_column]]
    y1 = df[a_column]
    model1_linear = sm.OLS(y1, x1).fit()
    
    # Model 2: Linear regression with cubic term
    x2 = df[[x_column, v_column, x_column_cubic]]
    y2 = df[a_column]
    model2_inverted_U = sm.OLS(y2, x2).fit()
    
    # Model 3: Linear regression with quadratic, cubic, and quartic terms
    x3 = df[[x_column, v_column, x_column_quadratic, x_column_cubic, x_column__quartic]]
    y3 = df[a_column]
    model3_S = sm.OLS(y3, x3).fit()
        
    # Collect results for the current stage
    results = [
        {
            'model': 'model1_linear',
            'r_squared': model1_linear.rsquared,
            'coefficients': model1_linear.params.to_dict(),
            'p_values': model1_linear.pvalues.to_dict()
        },
        {
            'model': 'model2_inverted_U',
            'r_squared': model2_inverted_U.rsquared,
            'coefficients': model2_inverted_U.params.to_dict(),
            'p_values': model2_inverted_U.pvalues.to_dict()
        },
        {
            'model': 'model3_S',
            'r_squared': model3_S.rsquared,
            'coefficients': model3_S.params.to_dict(),
            'p_values': model3_S.pvalues.to_dict()
        }
    ]
...
def generate_stiffness_graph(df, regression_results, r2_threshold=0.85):
    for model_result in regression_results.to_dict('records'):
        if model_result['r_squared'] >= r2_threshold:
            coefficients = model_result.get('coefficients', {})

            # Ensure all coefficients are float and not other types like string or unexpected structures
            def safe_get(key):
                value = coefficients.get(key, 0.0)
                try:
                    return float(value)
                except (TypeError, ValueError):
                    print(f"[⚠️] Invalid coefficient '{key}' with value: {value}. Replacing with 0.0")
                    return 0.0

            a = safe_get('zp_std')
            c = safe_get('zp_std_quadratic')
            d = safe_get('zp_std_cubic')
            e = safe_get('zp_std_quartic')
            ...
            print(f"Coefficients: a={a}, c={c}, d={d}, e={e}")
            print(f"Sample stiffness values: {stiffness_values[:5]}")

            plt.figure(figsize=(10, 10))
            plt.plot(x_values, stiffness_values, label='Stiffness Curve', color='black')
            plt.axhline(0, color='black', linestyle='--', linewidth=0.8)
            plt.xlabel('Displacement', color='black', fontsize=15)
            plt.ylabel('Stiffness', color='black', fontsize=15)
            plt.ylim(0, 2)
            plt.xticks(np.arange(-1, 2, 1), fontsize=20)
            plt.yticks(np.arange(0, 3, 1), fontsize=20)
            plt.title(f"Stiffness Model: {model_result['model']} (R²={model_result['r_squared']:.2f})", fontsize=18)
            plt.legend(fontsize=10)
            plt.grid(False)
            output_file_name = f"stiffness_graph_{model_result['model']}.png"
            plt.savefig(output_file_name, bbox_inches='tight')
            plt.show()
            print(f"Stiffness graph saved as {output_file_name}")
            return

    print("No model met the R-squared threshold.")
...
def process_file():
    ...
    try:
        ...
        if df is not None:
            ...
            # Generate stiffness graph
            generate_stiffness_graph(df_filtered2, summary)
